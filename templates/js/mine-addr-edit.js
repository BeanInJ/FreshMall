...
$(function() {
	...
	// 点击p时获取省级城市名单
	get_city(p, 0);
	get_all(token, parseInt(GetRequest().edit_addr_id));
	// p发生变化
	$(p).change(function() {
	...
	$("#btnSave").click(function() {
		...
		if (m == 0) {
			$.post("/static/addr/", {
				...
				"is_select":is_select,
				"get_name":"edit",
				"edit_addr_id":parseInt(GetRequest().edit_addr_id)
			}, function(data) {
				if(data["status"]==200){
					if(GetRequest().action){
						if (GetRequest().order_id) {
							var url_l = '/templates/mine-addr-list-cart.html?action=' + GetRequest().action+ "&order_id=" + GetRequest().order_id;
						} else {
							var url_l = '/templates/mine-addr-list-cart.html?action=' + GetRequest().action;
						}
						$(location).prop('href', url_l);
					}else{
...
//获取地址详细信息
function get_all(token,id){
	$.get("/static/addr/", {
		"token": token,
		"edit_addr_id":id
	}, function(data) {
		console.log(data)
		$("#order_name").attr("value",data[0]["a_user_name"]);
		$("#tell").attr("value",data[0]["a_tell"]);
		$("#addr_dail").attr("value",data[0]["a_detail"]);
	});
};
//获取url参数封装成对象
function GetRequest() {
